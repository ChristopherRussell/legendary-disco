...
// Grand prize is an all-expenses-paid trip to Desert Island!" That must be where the sand comes from! Best of all, the boat races are starting in just a few minutes.
...
// ß The boats are much smaller than you expected - they're actually toy boats, each with a big button on top
...
pub fn run() -> anyhow::Result<i64> {
    let (times, distances) = extract_input()?;
    println!("times: {:?}", times);
    ...
    let ways_to_win_product = ways_to_win.iter().product::<i64>();

    println!("ways_to_win_product: {:?}", ways_to_win_product);

    // In part 2, we assume the whitespace in the input is ignored and there is only one time and one distance.
    // So concatenate the times and distances into a single number. Then we return the number of races where we would
    // win.
    let time2 = times
        .into_iter()
        .map(|x| x.to_string())
    ...
    println!(
        "Second part race time: {:?}, distance: {:?}",
        time2, distance2
    );

    let race2 = Race::new(time2, distance2);
    print!("Second part ways to win: {:?}   ", race2.ways_to_win());
    Ok(ways_to_win_product)
}

fn extract_input() -> Result<(Vec<i64>, Vec<i64>), anyhow::Error> {
    let reader = INPUT;
    ...
    let distances = if let Some("Distance:") = second_line.next() {
        second_line
            .map(|x| x.parse::<i64>().unwrap())
            .collect::<Vec<i64>>()
    } else {
